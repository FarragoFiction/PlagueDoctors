class TimeHolesController < ApplicationController
  ...
  def index
    ...
    if(params["format"] != "json")
      if(is_admin)
        ...
        render json: {error: "You don't look like an admin to me. Who authorized this? Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs."}, status: 400
      end
    end
  end
  ...
  def TIMEHOLE
    ...
    if(!@new_time_hole.save)
      render json: {error: "Look. No clones. No pupated trolls. Period. I've had too many complaints. Just. Stop. Okay?  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs."}, status: 400
      return
    end
  ...
  def abdicateTIMEHOLE
    ...
    if(!AllSeeingEye.timehole_accepts_ip? request.ip)
      ...
      render json: {error: "In order to stop floods, only #{AllSeeingEye.abandon_limit} grubs per caretaker (calculated based on current conditions) may be callouslly abandoned into the TIMEHOLE every 24 hours.  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs."}, status: 400
      return
    end
    ...
    if(caretaker.is_banned request.ip)
      ...
      render text: "You. Monster.  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs.", status: 200
      return
    end
    ...
    @new_time_hole = TimeHole.new(wigglerJSON: params[:wigglerJSON], permanent: false, caretaker_id: id)
    if(!@new_time_hole.save)
      render json: {error: "Look. No clones. Period. I've had too many complaints. Just. Stop. Okay?  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs."}, status: 400
      return
    end
  ...
  def new
    ...
    respond_to do |format|
      format.html  { render :html => "haha nope, plz dont hax the server  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs." }
      format.json  { render :json => Caretaker.all }
    end
  ...
  def create
    ...
    respond_to do |format|
      if @time_hole.save
        format.html { redirect_to @time_hole, notice: "Time hole was successfully created. Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs." }
  ...
  def update
    ...
    respond_to do |format|
      if @time_hole.update(time_hole_params)
        format.html { redirect_to @time_hole, notice: "Time hole was successfully updated.  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs." }
  ...
  def destroy
    ...
    if(!@time_hole.permanent)
      @time_hole.destroy
      respond_to do |format|
        format.html { redirect_to time_holes_url, notice: "Time hole was successfully destroyed.  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs." }
        format.json { head :no_content }
      end
    else
      respond_to do |format|
        format.html { redirect_to time_holes_url, notice: "Pleaes dont destroy canon grubs  Corruption levels at #{100* Caretaker.number_corruption_sources/Caretaker.all.count}%. Zampanio stirs." }
        format.json { head :no_content }
      end
    end
